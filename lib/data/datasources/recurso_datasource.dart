...
class RecursoDatasource {
  final _db = FirebaseFirestore.instance;
  final _storage = FirebaseStorage.instance;

  Stream<List<RecursoModel>> getRecursos(String grupoId) =>
      _db.collection('grupos').doc(grupoId).collection('recursos')
  ...
  Future<String> uploadArchivo(
    ...
    final ref = _storage.ref(
        'grupos/$grupoId/recursos/${DateTime.now().millisecondsSinceEpoch}.$ext');
  ...
  Future<void> createRecurso({
    ...
    String? nombreArchivo,
      ...
      _db.collection('grupos').doc(grupoId).collection('recursos').add({
        'grupoId': grupoId,
        'autorUid': autorUid,
